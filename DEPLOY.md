...
> ✅ **Kalıcılık:** PDF blob'ları ve meta veri index'i (`STORAGE_PATH/_index.json`) diske yazılır
> ve açılışta geri yüklenir; `STORAGE_PATH` kalıcı (mounted volume) olduğu sürece yeniden
> başlatmada `file_id`'ler korunur. Tek instans için Postgres gerekmez (HA/çok-instans isterseniz
> Faz‑2'de eklenir).
...
- [ ] `STORAGE_PATH` kalıcı bir volume'a bağlı (blob + index orada saklanır)
